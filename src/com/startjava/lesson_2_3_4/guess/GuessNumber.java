...
//    private Player player2;
    ...
    public void start() {
        ...
        for (int i = 1; i <= COUNT_ROUNDS_CONSTANT; i++) {
            ...
            // случайный индекс (из 0,1,2) на последнее место, следующий случайный индекс (из 0,1) на предпоследнее место
            Player temp;
            ...
            System.out.println(players[0].getName() + ", " + players[1].getName() + " и " + players[2].getName());
            ...
            try {
                for (int j = 0; j < COUNT_ATTEMPTS_CONSTANT; j++) {
                    for (int k = 0; k < players.length; k++) {
                        //System.out.print("Игрок " + players[k].getName() + ": ");   //+ " делает попытку : "
                        ...
                        do {
                            try {
                                ...
                                if((enteredNum > 0) && (enteredNum <= 100)) {
                                    ...
                                    System.out.println("Вводимое целое число должно между 0 и 100, делайте попытку еще");
                        ...
                        if (enteredNum == guessedNum) {
                            System.out.println("Игрок " + players[k].getName() + " угадал " + guessedNum + " с " +
                                    ++j + " попытки");
                            ...
                            String text = "Закончился раунд " + String.valueOf(i);
                            throw new FinishRoundException(text);
                        } else {
                            System.out.println("Число " + enteredNum + (enteredNum < guessedNum ? " меньше" : " больше") +
                                    " загаданного");
                        }
                        //currentAttempt++;
        ...
        if (max == min) {
            ...
            int  winnerCount = 0;
            for (int j = 0; j < players.length; j++) {
                if (players[j].getWinsCount() == max) {
                    winnerCount++;
                    System.out.println(winnerCount > 1 ? ",и победил " : "Победил " + players[j].getName() );
                    break;
                }
            }
        }
    ...
    public void PrintResult(int attemptСount, int[] Numbers) {
        System.out.println("Все названные числа:");
        int half = (int) Math.ceil((float) attemptСount / 2) - 1;
        for (int i = 0; i < attemptСount; i++) {
    ...
    public void PrintPlayer(int indexPlayer) {
        ...
        if(players[indexPlayer].getAttempt() > 0) {
            int[] numbersToPrint = Arrays.copyOf(players[indexPlayer].getNumbers(), players[indexPlayer].getAttempt());
    ...
    public void ClearArrays() {
        Arrays.fill(namedNumbers, 0);
        for (int i = 0; i < COUNT_PLAYERS_CONSTANT; i++) {
...
//        int[] numbersToCopy = java.util.Arrays.copyOf(namedNumbers, half);
//        for (int number : numbersToCopy) {
//            System.out.print(number + " ");
//        }
//        int[] numbers2ToCopy = java.util.Arrays.copyOfRange(namedNumbers, half + 1, num);
//        for (int number : numbers2ToCopy) {
//            System.out.print(number + " ");
//        }
